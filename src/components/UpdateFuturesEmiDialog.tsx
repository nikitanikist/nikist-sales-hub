...
export function UpdateFuturesEmiDialog({
  open,
  onOpenChange,
  studentId,
  offerAmount,
  ...
  const [editingEmi, setEditingEmi] = useState<FuturesEmiPayment | null>(null);
  ...
  // Handler for new EMI payment platform change - recalculates platform fees
  ...
  // Handler for edit EMI payment platform change - recalculates platform fees
  ...
  const { data: emiPayments, isLoading: isLoadingEmi } = useQuery({
    queryKey: ["futures-emi-payments", studentId],
    queryFn: async () => {
  ...
  const { data: offerAmountHistory } = useQuery({
    queryKey: ["futures-offer-history", studentId],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("futures_offer_amount_history")
  ...
  const recalculateStudent = async (emiPaymentsList: FuturesEmiPayment[]) => {
  ...
  const handleDeleteEmi = async () => {
    ...
    try {
      ...
      queryClient.invalidateQueries({ queryKey: ["futures-emi-payments", studentId] });
      queryClient.invalidateQueries({ queryKey: ["futures-students"] });
      queryClient.invalidateQueries({ queryKey: ["futures-emi"] });
  ...
  const handleSaveAll = async (options: { closeAfterSuccess: boolean }) => {
    ...
    try {
      ...
      if (offerAmountChanged) {
        ...
        await supabase.from("futures_offer_amount_history").insert({
          student_id: studentId,
      ...
      queryClient.invalidateQueries({ queryKey: ["futures-emi-payments", studentId] });
      queryClient.invalidateQueries({ queryKey: ["futures-offer-history", studentId] });
      queryClient.invalidateQueries({ queryKey: ["futures-students"] });
      queryClient.invalidateQueries({ queryKey: ["futures-emi"] });
