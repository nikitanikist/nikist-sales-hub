...
-- Recreate function with call_appointment_id column
CREATE OR REPLACE FUNCTION public.get_workshop_sales_leads(p_workshop_title text)
 RETURNS TABLE(id uuid, lead_id uuid, contact_name text, email text, phone text, scheduled_date date, scheduled_time time without time zone, status text, closer_name text, has_call_appointment boolean, call_appointment_id uuid)
 LANGUAGE plpgsql
 SECURITY DEFINER
