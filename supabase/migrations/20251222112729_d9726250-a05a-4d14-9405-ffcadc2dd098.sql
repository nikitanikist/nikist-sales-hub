-- Update get_workshop_metrics to prioritize lead_assignments.workshop_id over leads.workshop_name
...
AS $function$
  SELECT 
    ...
    -- Total calls booked = Workshop Sales (people who purchased the ₹497 product)
    (
      SELECT COUNT(DISTINCT la1.lead_id)
      FROM lead_assignments la1
      INNER JOIN lead_assignments la2 ON la2.lead_id = la1.lead_id
