-- Update get_workshop_metrics to only count calls from Workshop Sales
...
AS $function$
  SELECT 
    ...
    -- Converted calls: ONLY from Workshop Sales (leads with both workshop + product)
    (
    ...
    -- Rescheduled Done: ONLY from Workshop Sales
    (
      SELECT COUNT(DISTINCT ca.id)
      FROM call_appointments ca
      INNER JOIN leads l ON l.id = ca.lead_id
    ...
    -- Total offer amount: ONLY from Workshop Sales converted calls
    (
    ...
    -- Total cash received: ONLY from Workshop Sales converted calls
    (
...
-- Update get_workshop_calls_by_category to only return calls from Workshop Sales
