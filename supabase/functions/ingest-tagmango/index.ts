...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ============ DYNAMIC RULE LOOKUP (additive — falls back to hardcoded logic) ============
    let handledByDynamicRule = false;

    if (mangoId) {
      try {
        // Look up a dynamic registration confirmation rule for this trigger_id
        ...
        console.error('Unexpected error in dynamic rule lookup:', ruleErr);
        // Fall through to hardcoded logic
      }
    }

    // ============ HARDCODED FALLBACK (only runs if no dynamic rule matched) ============
    if (!handledByDynamicRule) {
